            ...
            case (state)
                S_IDLE: begin
                    ...
                    if (start) begin
                        ...
                        out_x <= 16'd0;
                        out_y <= 16'd0;
                        // Only reset counters at the START of a new operation
                        progress <= 32'd0;
                        perf_mem_reads <= 64'd0;
                        perf_mem_writes <= 64'd0;
                        perf_pixel_reuse <= 64'd0;
                    ...
                    // If not starting, preserve progress value from completed operation
                end
                ...
                S_FETCH: begin
                    ...
                    if (fetch_lane >= batch_size) begin
                        ...
                        if (do_full_reuse) begin
                            ...
                            read_pending <= 1'b1;  // Set immediately when issuing read
                        end
                    end else begin
                        // Clear read signal after it's been issued
                ...
                S_WRITE: begin
                    if (batch_idx < batch_size && lane_valid[batch_idx]) begin
                        if (!write_pending) begin
                            ...
                            write_pending <= 1'b1;  // Set immediately when issuing write
                            perf_mem_writes <= perf_mem_writes + 1;
                        end else begin
                            sdram_write <= 1'b0;  // Clear write signal after one cycle
                            write_pending <= 1'b0;
