...
module simd_downscaler_tb;
    ...
    initial begin
        ...
        // Unified test suite (skipping very small tests since LANES=4)
        // Note: 2x2 and 4x4 outputs are smaller than LANES, skipped
        ...
        $display("\n========================================");
        $display("All SIMD (LANES=4) tests completed!");
        $display("========================================\n");
        $finish;
    end
    ...
    task automatic run_test(
        ...
        // Calculate scale factor matching hardware: (width-1)/(out_width-1) in Q8.8
        if (OW > 1)
            scale_q8_8 = ((IW - 1) << Q) / (OW - 1);
        else
            scale_q8_8 = 0;
    ...
    // Capture outputs in a separate always block
    int out_x = 0, out_y = 0;
    always_ff @(posedge clk) begin
        if (out_valid) begin
            for (int l = 0; l < LANES; l++) begin
                if (out_x < out_width) begin
                    image_out_mem[out_y][out_x] = out_pixels_packed[l*PIXEL_WIDTH +: PIXEL_WIDTH];
                    out_x++;
                    if (out_x >= out_width) begin
                        out_x = 0;
                        out_y++;
    ...
    task automatic process_image(
        ...
        int y, x, l;
        int total_outputs, timeout;
        
        // Reset output capture
        out_x = 0;
        out_y = 0;
        
        // Pulse start once at the beginning
        start = 1;
        @(posedge clk);
        start = 0;
        
        // Drive all inputs continuously
        for (y = 0; y < OH; y++) begin
            for (x = 0; x < OW; x = x + LANES) begin
                ...
                @(posedge clk); // Advance one clock cycle with new inputs driven
            end // for (x)
        end // for (y)
        ...
        total_outputs = OW * OH;
        timeout = 0;
        while (out_y * OW + out_x < total_outputs && timeout < 10000) begin
            @(posedge clk);
            timeout++;
        end
        
        if (timeout >= 10000) begin
            $display("ERROR: Timeout waiting for all outputs. Got %0d/%0d pixels", out_y * OW + out_x, total_outputs);
            $finish;
        end
    endtask
    ...
    task automatic generate_reference(
        ...
        // Match hardware scale calculation: (width-1)/(out_width-1)
        if (OW > 1)
            scale_factor_q = ((IW - 1) << Q) / (OW - 1);
        else
            scale_factor_q = 0;
