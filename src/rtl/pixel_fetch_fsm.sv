...
// for both SIMD (8 lanes) and Serial (1 lane) modes
//=======================================================
...
module pixel_fetch_fsm #(
    ...
    output logic [7:0]  dbg_frac_x,
    output logic [7:0]  dbg_frac_y,
    output logic [7:0]  dbg_p00,
    ...
    logic [3:0]  active_lanes;      // LANES for SIMD, 1 for serial
    ...
    //=======================================================
    // Active lanes based on mode
    //=======================================================
    assign active_lanes = (mode == 2'd1) ? 4'd1 : LANES[3:0];
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_COMPUTE_COORD: begin
                    ...
                    for (int i = 0; i < LANES; i++) begin
                        if (i < active_lanes) begin
                            // src = out / scale = out * (256 / scale_q8_8)
                            // Using: src_q8 = (out << 16) / scale_q8_8
                            logic [31:0] out_x_lane;
                            out_x_lane = out_x + i;
                            src_x_q8[i] <= (out_x_lane << 16) / scale_q8_8;
                            src_y_q8[i] <= (out_y << 16) / scale_q8_8;
                        end
                    end
                    current_lane <= 4'd0;
                end
                ...
                S_PACK_PIXELS: begin
                    ...
                    lb_rd_col <= src_x_q8[current_lane][23:8]; // Integer part
                    ...
                    lane_frac_x[current_lane] <= src_x_q8[current_lane][7:0];
                    lane_frac_y[current_lane] <= src_y_q8[current_lane][7:0];
                ...
                S_WRITE_PIXEL: begin
                    ...
                    write_pixel <= result_pixels[write_lane*8 +: 8];
                    write_addr <= img_out_addr + (out_y * out_width) + out_x + write_lane;
                    
                    sdram_write <= 1'b1;
                    sdram_address <= write_addr;
                    // Pack 2 pixels into 16-bit write when possible
                    if (!write_byte_sel) begin
                        write_buffer <= write_pixel;
                        sdram_writedata <= {8'd0, write_pixel};
                        write_byte_sel <= 1'b1;
                    end else begin
                        sdram_writedata <= {write_pixel, write_buffer};
                        write_byte_sel <= 1'b0;
                    end
                end
    ...
    //=======================================================
    // SDRAM Interface - Byte Enable always full
    //=======================================================
    ...
    assign dbg_src_x_int = src_x_q8[dbg_lane_sel][23:8];
    assign dbg_src_y_int = src_y_q8[dbg_lane_sel][23:8];
    ...
    assign dbg_out_pixel = result_pixels[dbg_lane_sel*8 +: 8];
